...
superagent
  ...
  .then((res) => {
    ...
    var metas = [];
    document.querySelectorAll('.lastest_update div.list a.la').forEach((a) => {
      var url = a.href
      ...
      metas.push({
        ...
        time: dayjs.utc(a.querySelector('.time').textContent, 'YYYY-MM-DD HH:mm').toDate(),
      })
    })
    ...
    superagent
      ...
      .then((res) => {
        ...
        var drama = {
          ...
          cover: document.querySelector('.resource_content div.image a').getAttribute('href'),
          meta: {},
          intro: document.querySelector('.resource_intro .intro_text').textContent,
          episodes: [],
        }

        document.querySelectorAll('.resource_content div.info ul li').forEach((li) => {
        ...
        document.querySelectorAll('.resource_item ul.item_list li').forEach(item => {
          var ep = { links: [] }
          ...
          if (meta.season == ep.id && meta.name == ep.title) {
            ...
            contentLines.push("\n\n\n\n" + description)
            // contentLines.push("generated byZhuixinfan updater")
            resolve(
              {
